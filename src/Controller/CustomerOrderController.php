...
class CustomerOrderController extends AbstractController implements PullInterface {
    ...
    public function pull(JTLModel\QueryFilter $queryFilter) : array {
        ...
        $merchant_id = (int) $this->query("SELECT value FROM options WHERE name = ?",['vivino.merchant_id'])->fetch(\PDO::FETCH_COLUMN,0);
        ...
        foreach ( $orders as $order ) {
            ...
            $address = (new JTLModel\CustomerOrderShippingAddress("{$order->order_id}-addr"))
            ...
            // order
            $jtlOrder = (new JTLModel\CustomerOrder( $order->order_id ))
                ...
                ->setPaymentStatus( JTLModel\CustomerOrder::PAYMENT_STATUS_COMPLETED ) // <= TODO: take from config
            ...
            foreach ( $order->positions as $i => $position ) {
                $jtlPosition = (new JTLModel\CustomerOrderItem("{$order->order_id}-item-{$i}"))
                    ...
                    ->setPriceGross($position->item_total_amount)
                    ;
            ...
            if ( $order->items_shipping_sum ) {
                $position = (new JTLModel\CustomerOrderItem("{$order->order_id}-shipping"))
            ...
            if ( $discount > 0 ) {
                $position = (new JTLModel\CustomerOrderItem("{$order->order_id}-discount"))
    ...
    private function getOrders(?int $query_limit) {
        ...
        $states       = ['Approved','Confirmed','Shipped'];
        $placeholders = implode(',',array_fill(0,count($states),'?'));

        $sql = "SELECT DISTINCT {$select}
            FROM vivino_order_positions
            WHERE jtl_id IS NULL AND status IN ({$placeholders})
            GROUP BY order_id
        ...
        return array_map(
            ...
            $this->query($sql,$states)->fetchAll(\PDO::FETCH_OBJ)
        );
    }
    ...
    private function processVivinoOrder(object $order) {
        ...
        $order->positions = array_map(
            ...
            $this->query($sql,[$order->order_id])->fetchAll(\PDO::FETCH_OBJ)
        );
